...
class MyRecommendViewModel(application: Application) : AndroidViewModel(application) {
    ...
    fun initUsers(){
        ...
        GlobalScope.launch(Dispatchers.Main) {
            ...
            recommendList.value = changeToRecommendList(response.value!!.data!!,usersListTest)
        }
    }
    ...
    fun changeToRecommendList(list1 : List<AccompanyAnime>,list2 : List<UserBean>): ArrayList<RecommendBean> {
        var list0 = ArrayList<RecommendBean>()
        for (i in 0 until list1.size){
            var t = list1[i]
            var user = list2[i]
            list0.add(RecommendBean(null,null,null,null,null,user.userid,user.name,null,null,t.dramaid,t.photo,null,null,t.title))
        }
        return list0
    }
